...
function vinfo = video_open(filename, cache_size)
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      %-----------------------------------------------------
      % Original code had the mmread/VideoReader the other way around. I guess in earlier
      % tests when this was written mmread was faster, but this does not seem to be the case
      % anymore. I find that VideoReader is much faster than mmread when playing back mp4 videos,
      % so I've flipped the try/catch blocks to try VideoReader before mmread.

      try %% Use VideoReader (mmread is terribly slow with mp4 videos)
          filename = absolute_path(filename);
          % store name and type
          vinfo.filename = filename;
          vinfo.type = 'vidobj';
          ...
          vinfo.sz = 1;
      catch %% In case VideoReader fails, use mmread
          % store name and type
          vinfo.filename = filename;
          vinfo.type = 'mmread';
          ...
          if (fid == -1)
             ...
             if ~isempty(strfind(header,'CNCVCanonAVC0005'))
                 ...
                 error('mmreader fails for CNCVCanonAVC0005 files');
             end
             fclose(fid);
          end
